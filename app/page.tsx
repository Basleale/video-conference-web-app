...
export default function Page() {
  ...
  return <main className="conference-shell">
    <header className="topbar"><div className="brand"><span className="brand-mark"><Sparkles size={15} /></span><span>luma<span className="brand-dot">.</span></span></div><div className="room-meta"><span className="live-dot" /> <span>Live</span><span className="room-divider" /> <span>{time}</span><span className="room-code">{roomCode}</span></div><div className="top-actions"><button className="icon-button" aria-label="Settings"><Settings size={18} /></button><button className="avatar">{(name || 'Y').slice(0, 1).toUpperCase()}</button></div></header>
